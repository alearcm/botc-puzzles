...
## 10. Ruleset switches and scoped principles

The almanac is a set of worked examples generated by unstated general
principles; the free parameter is induction distance — how far a cited
instance generalizes. The registry therefore has three layers:

- **Instances** — cited interactions. Hard constraints: every candidate
  principle must reproduce them; each becomes a regression test.
- **Scoped principles** — quantified rules with an explicit *scope variable*:
  an ordered ladder of candidate extents, a default, the instances entailed
  at each rung, and a contested-above-here line. What is contested is
  usually not "A or B" but the reach of a quantifier.
- **Switches** — the degenerate two-rung case (binary contested rulings).

Every rule is tagged `settled(citation)` | `switch(options, default)` |
`principle(scope ladder, default)` | `open`. A puzzle declares
`(script, switch/scope assignment)`. Well-posedness: a puzzle whose answer
varies across assignments of undeclared switches or plausible undeclared
scopes is flagged ill-posed. Registry: `RULINGS.md`.
