...
| `dead-mutant-claims-truthfully` | CLAIM CONVENTION, not a game rule: NQT rules text exempts only a "living Mutant" from truthful reporting, so a dead Mutant falls under "other good players truthfully report" — the mechanical basis is just `dead-lose-abilities` (the might-be-executed pressure is the ability). NOT a general madness mechanic: talk is free, and a dead player may still choose to comply (Cerenovus can target the dead; executing a dead player still ends the day). Applies only where the puzzle's convention text says so. | NQT #55 rules text + author comment |
...
| `drunk-token-not-in-play` | The Drunk's believed character is drawn from the NOT-in-play townsfolk (no in-play token) — author-stated assumption for the puzzle corpus, acknowledged grey area in official rules. Characters with legitimate duplicates (dup_ok: Village Idiot) are exempt: a Drunk-as-VI may coexist with real VIs. | NQT #36 author ruling + #20 author comment |
| `deaths-read-at-position` | Life state obeys the site clock: a reader at position I sees deaths that resolved at earlier positions — an Empath acting after the Imp reads the circle with tonight's victim already skipped. | Night-order semantics; NQT #31 confirming instance |
| `actions-read-at-position` | Action targets obey the same clock as info reads: the Snake Charmer's demon check evaluates at the acting HOLDER's slot (a philosopher copy runs at the philosopher's slot, `hpos`), so two snake-charmer holders can't both hit one demon — after the first swap the demon has moved. | Night-order semantics; caught on NQT #11 (double-swap incoherence) |
| `barber-not-demon` | The Barber swap may not involve a demon, alive or dead: printed "(not another Demon)". | Barber ability text; caught on NQT #11 (dead Vortox swapped into a good player) |
| `claims-standing-record` | CLAIM CONVENTION: a displayed claim is a standing public record — it must be covered on EVERY day of the window (truthful current character, drunk charade, living Mutant, cere-mad that day, or evil that day). A single final-day madness cannot retroactively cover earlier good days. Truth/Mutant covers key on CURRENT character (a Mutant pit-hagged away loses the cover; a player turned into the Mutant gains it). | NQT #11 author adjudications ("keeps their story consistent until something else happens") |
| `madness-reveal-on-end` | CLAIM CONVENTION: cerenovus madness that ends or changes makes the target reveal they were targeted — all players, evil and dead included; absent a reveal in the claims, madness persists through the window. | NQT #11 rules text + author-endorsed thread reasoning |
| `first-person-ground-truth` | CLAIM CONVENTION: "You" know your own character and tokens — the first-person claim is fact, not coverable by madness or fabrication. | Puzzle genre semantics; caught on NQT #11 (cere-madded "you" leak) |
